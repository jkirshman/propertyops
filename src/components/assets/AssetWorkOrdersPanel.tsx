...
export function AssetWorkOrdersPanel({ assetId, canCreate }: { assetId: string; canCreate: boolean }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
      {canCreate ? (
        <Link href={`/work-orders/new?assetId=${assetId}`} className="button button-primary" style={{ alignSelf: "flex-start" }}>
          New Work Order
        </Link>
      ) : null}
      ...
      {loading ? (
        ...
        <p className="muted">No work orders reference this asset yet.</p>
      ) : (
