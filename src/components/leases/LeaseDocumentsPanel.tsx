...
export function LeaseDocumentsPanel({ leaseId, canManage }: { leaseId: string; canManage: boolean }) {
  ...
  const load = useCallback(async () => {
    try {
      const response = await fetch(`/api/files?relatedEntityType=lease&relatedEntityId=${leaseId}`);
      if (response.ok) {
  ...
  async function handleUpload(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      formData.append("relatedEntityType", "lease");
      formData.append("relatedEntityId", leaseId);
      if (title.trim()) {
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
      {canManage ? (
        <form onSubmit={handleUpload} className="card" style={{ display: "flex", flexDirection: "column", gap: "0.75rem" }}>
          ...
          <input
            className="input"
            placeholder="Title (optional) — e.g. Signed Lease, Amendment"
            value={title}
