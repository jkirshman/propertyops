...
export function EquipmentDocumentsPanel({
  ...
  const load = useCallback(async () => {
    try {
      const response = await fetch(
        `/api/files?relatedEntityType=property_equipment&relatedEntityId=${propertyEquipmentId}`,
      );
      if (response.ok) {
  ...
  async function handleUpload(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      formData.append("relatedEntityType", "property_equipment");
      formData.append("relatedEntityId", propertyEquipmentId);
      if (title.trim()) {
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
      {canManage ? (
        ...
        >
          ...
          <input
            className="input"
            placeholder="Title (optional) — e.g. Manual, Warranty, Invoice"
            value={title}
