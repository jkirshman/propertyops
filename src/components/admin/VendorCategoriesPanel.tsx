...
export function VendorCategoriesPanel() {
  const [categories, setCategories] = useState<VendorCategoryRecord[]>([]);
  ...
  async function handleCreate(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!response.ok) {
        setError(data?.details?.formErrors?.[0] ?? "Could not create the vendor category.");
        return;
      }
      setName("");
      setDescription("");
  ...
  async function toggleActive(category: VendorCategoryRecord) {
    await fetch(`/api/vendor-categories/${category.id}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "1.25rem" }}>
      ...
      >
        ...
        >
          {submitting ? "Adding…" : "Add vendor category"}
        </button>
      </form>
      ...
      <div className="card">
        {loading ? (
          ...
          <p className="muted">No vendor categories yet.</p>
        ) : (
