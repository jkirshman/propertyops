...
export function TenantDocumentsPanel({ tenantId, canManage }: { tenantId: string; canManage: boolean }) {
  ...
  const load = useCallback(async () => {
    try {
      const response = await fetch(`/api/files?relatedEntityType=tenant&relatedEntityId=${tenantId}`);
      if (response.ok) {
  ...
  async function handleUpload(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      formData.append("relatedEntityType", "tenant");
      formData.append("relatedEntityId", tenantId);
      if (title.trim()) {
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
      {canManage ? (
        <form onSubmit={handleUpload} className="card" style={{ display: "flex", flexDirection: "column", gap: "0.75rem" }}>
          ...
          <input
            className="input"
            placeholder="Title (optional) — e.g. Identification, Insurance"
            value={title}
        ...
        <ul style={{ listStyle: "none", display: "flex", flexDirection: "column", gap: "0.6rem" }}>
          {files.map((file) => (
            <li key={file.id} className="card" style={{ display: "flex", justifyContent: "space-between", gap: "1rem" }}>
              <span>
                {file.title ?? file.fileName} <span className="muted">({Math.round(file.sizeBytes / 1024)} KB)</span>
              </span>
              <a href={`/api/files/${file.id}`} className="button">Download</a>
            </li>
          ))}
        </ul>
      )}
