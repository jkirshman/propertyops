...
export function WorkOrderAttachmentsPanel({
  ...
  const load = useCallback(async () => {
    try {
      const response = await fetch(
        `/api/files?relatedEntityType=work_order&relatedEntityId=${workOrderId}`,
      );
      if (response.ok) {
  ...
  async function handleUpload(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      formData.append("relatedEntityType", "work_order");
      formData.append("relatedEntityId", workOrderId);
