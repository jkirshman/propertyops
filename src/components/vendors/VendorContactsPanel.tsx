...
export function VendorContactsPanel({ vendorId, canManage }: { vendorId: string; canManage: boolean }) {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    setSubmitting(true);
    try {
      const url = editingId ? `/api/vendors/${vendorId}/contacts/${editingId}` : `/api/vendors/${vendorId}/contacts`;
  ...
  async function toggleActive(contact: ContactRecord) {
    await fetch(`/api/vendors/${vendorId}/contacts/${contact.id}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
          ...
          {showForm ? (
            ...
            >
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: "0.75rem" }}>
                ...
                <div>
                  <label className="label" htmlFor="vendor-contact-title">
                    Role / title
                  </label>
                  <input
                ...
                <div>
                  <label className="label" htmlFor="vendor-contact-phone">
                    Phone
                  </label>
                  <input
                ...
                <div>
                  <label className="label" htmlFor="vendor-contact-mobile">
                    Mobile (optional)
                  </label>
                  <input
                ...
                <div style={{ display: "flex", alignItems: "flex-end", gap: "0.4rem" }}>
                  <input
                    id="vendor-contact-primary"
                    type="checkbox"
                    checked={form.isPrimary}
                    onChange={(event) => setForm((prev) => ({ ...prev, isPrimary: event.target.checked }))}
                  />
                  <label htmlFor="vendor-contact-primary">Primary contact</label>
                </div>
              </div>
              <div>
                <label className="label" htmlFor="vendor-contact-notes">
                  Notes
                </label>
                <textarea
                  id="vendor-contact-notes"
                  className="input"
                  rows={2}
                  value={form.notes}
