...
export function VendorDocumentsPanel({ vendorId, canManage }: { vendorId: string; canManage: boolean }) {
  ...
  const load = useCallback(async () => {
    try {
      const response = await fetch(`/api/files?relatedEntityType=vendor&relatedEntityId=${vendorId}`);
      if (response.ok) {
  ...
  async function handleUpload(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      formData.append("relatedEntityType", "vendor");
      formData.append("relatedEntityId", vendorId);
      if (title.trim()) {
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
      {canManage ? (
        ...
        >
          ...
          <input
            className="input"
            placeholder="Title (optional) — e.g. W-9, Certificate of Insurance, Contract"
            value={title}
