...
export function PropertyComponentWorkOrdersPanel({
  propertyComponentId,
  canCreate,
}: {
  ...
  useEffect(() => {
    fetch(`/api/work-orders?propertyComponentId=${propertyComponentId}`)
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
      {canCreate ? (
        <Link
          href={`/work-orders/new?componentId=${propertyComponentId}`}
      ...
      {loading ? (
        ...
        <p className="muted">No work orders reference this component yet.</p>
      ) : (
